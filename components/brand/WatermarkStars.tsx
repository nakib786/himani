...
/**
 * KSHYOVRATA — footer star drift
 *
 * The four-pointed sparkle from the logo's celestial cluster, set loose across
 * the oversized footer wordmark. Three independent motions per star, each on
 * its own element so they compose instead of fighting over one `transform`:
 *
 *   .star        travels the full width, edge to edge
 ...
 * right-to-left. Read together, the two streams circulate around the word
 ...
 * the animations start mid-cycle, so the stars are already spread across the
 * word on first paint instead of queueing up at one edge.
 *
...
type Star = {
  /** Vertical position, as a percentage of the wordmark box. */
  lane: string;
